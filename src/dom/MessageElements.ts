...
abstract class AssistantResponse {
  ...
  private static syntheticMessageIdCounter = 0;
  ...
  get messageId(): string {
    // Stable identifier for this assistant message. Default implementation
    // relies on a synthetic value that persists for the lifetime of the DOM
    // node. Subclasses may override this getter if the host provides a native
    // identifier (see ChatGPTResponse).
    if (this._messageId) {
      return this._messageId;
    }

    this._messageId = this.ensureSyntheticMessageId();
    return this._messageId;
  }

  private ensureSyntheticMessageId(): string {
    const existing = this._element.dataset.saypiMessageId;
    if (existing && existing.trim().length > 0) {
      return existing.trim();
    }

    const generated = AssistantResponse.generateSyntheticMessageId();
    this.safeSetDataAttribute("saypiMessageId", generated);
    return generated;
  }

  private static generateSyntheticMessageId(): string {
    const globalCrypto = (globalThis as unknown as { crypto?: Crypto }).crypto;
    if (globalCrypto?.randomUUID) {
      return globalCrypto.randomUUID();
    }
    const counter = (AssistantResponse.syntheticMessageIdCounter++).toString(16);
    return `saypi-msg-${Date.now().toString(16)}-${counter}`;
  }
  ...
  get utteranceId(): string | null {
    // Ephemeral identifier generated by SayPi when we create a TTS stream. A
    // single message can accumulate multiple utterance ids over time; this only
    // returns the id of the most recently attached stream (if any).
