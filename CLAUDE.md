...
- **Layer 4 (CDP) — autonomous Claude/ChatGPT, unattended** (`npm run layer4cdp:verify`). Spawns the founder's **real Chrome** directly + attaches over CDP, so it loads the extension AND passes Cloudflare (the combo Layer 3.5 can't) for **claude.ai/chatgpt.com** with no founder/MCP. One-time `layer4cdp:seed` (login) + `layer4cdp:diagnose` (measures `cf_clearance` upkeep). On-demand, **not** CI. See **[doc/layer4-cdp-real-host-loop.md](doc/layer4-cdp-real-host-loop.md)**.
