...
- **E2E dictation sweep — real-host defect hunt for universal dictation** (`npm run e2e-dictation-sweep`). The Layer-4 (CDP) counterpart to the host sweep, but for `UniversalDictationModule.ts` — the per-field-button dictation feature on arbitrary (non-chat-host) sites, which the host sweep never exercises. Drives a synthetic dictation turn into the local test fixture (input/textarea/contenteditable) and Mistral's Le Chat (real, login-free, ProseMirror composer) and checks the transcript landed. No chat concepts (no model/voice select, no TTS). Backs the `/e2e-dictation-sweep` skill. On-demand, **not** CI. See **[doc/e2e-dictation-sweep.md](doc/e2e-dictation-sweep.md)**.
- **Sweep — combined defect hunt** (no dedicated npm script; runs the two above sequentially). The local `/sweep` skill is a thin orchestrator that runs both `e2e-host-sweep` and `e2e-dictation-sweep` back-to-back with one combined report — use it for a general "sweep for bugs" ask that doesn't name a specific host or feature.
