...
**Layout.** Do **not** add a Voices-only width rule — `voices.css:12-22` carries an explicit warning and #582/#583 were both regressions on this exact tab. Settings now open in a **browser tab**, not a sized popup (#584), and the adaptive window-sizing machinery it made unreachable has been deleted (#587) — so there is no window to grow any more. The rail's width is whatever the tab's content column gives it, capped by `#voice-studio`'s own `max-width: 900px`. If the rail ever needs more room, change that cap.
