...
- **Stack**: TypeScript/ESM browser extension built with **WXT**. npm (not pnpm), Node 22 (`.nvmrc`). Targets Chrome/Edge **MV3** (`npm run dev`) and Firefox **MV2** (`npm run dev:firefox`). Voice activity detection via `onnxruntime-web`/VAD; Web Audio throughout. **XState v5** state machines already in the stack (`xstate@^5`; see `src/state-machines/README.md`). `@xstate/fsm` and `@xstate/test` were removed in the 2026-06 v4→v5 migration (#401/#402); the model-based path-testing tool is now `@xstate/graph` (planned, tracked in #404).
...
3. **Add headless E2E against a mock platform page.** Stand up a Playwright (or WXT-native) harness that loads the built extension against a *local mock* chat page — exercising the real content-script/background/offscreen wiring without depending on any live third-party site. This is the layer CI can run on every PR. **SHIPPED (this PR):** a Playwright harness loads the development-mode static build into headless Chrome, redirects real saypi hostnames to local mock HTTPS servers, and asserts both Pi-page decoration and a fake-audio mic → real WASM-VAD → mock STT transcript landing in the prompt — wired into CI as an *advisory* job. See the runbook `e2e/README.md` (design rationale in `doc/specs/2026-06-14-layer3-headless-e2e-design.md`).
4. **Reserve real-site E2E for spot-checks.** Use the **Claude-in-Chrome** browser automation (available to you as an MCP) to smoke-test against the genuine Pi/Claude/ChatGPT sites — as a periodic/manual confidence pass, not a per-PR gate, since real sites are flaky and rate-limited by nature. **SHIPPED (PR #287):** the autonomous dev-verify loop is `scripts/dev-rig.mjs` + Claude-in-Chrome — see the runbook `doc/autonomous-dev-loop.md`.
