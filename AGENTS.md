...
- **File issues on the sibling repos, and open (never merge) coordinated PRs there** — `saypi-api` and `saypi-saas`. Filing issues was granted 2026-06-13; opening PRs was granted 2026-07-08 (issue #528), under five guardrails: (1) **open-only** — the server repo's own merge gates, reviewers and constitution govern entirely, and you never merge, approve, close or relabel anything there beyond your own PR; (2) **coordinated pairs** — each side backlinks the other, states which side can ship first, and stays independently revertible, so there is no flag day; (3) **same hygiene as home** — isolated branch, a claim comment on the target repo's issue, full provenance, never touch another agent's work there, never read that repo's production secrets; (4) **contract-adjacent scope only** — the endpoints, fields and headers this client consumes plus the tests and docs they need, not general feature work in the server repos; (5) **no tunnelling** — whatever the server repo's own constitution founder-gates stays founder-gated there.
