...
- `.cursor/` — **Legacy** Cursor IDE rules in `rules/*.mdc` (written Nov 2025, pre-WXT) and MCP config in `mcp.json`. ⚠️ The rules still describe the Webpack-era build (`webpack.config.js`, a hand-maintained `manifest.json`) — wrong since the WXT migration. Don't trust their build/manifest claims; this file and `CLAUDE.md` supersede them. No runtime effect.
