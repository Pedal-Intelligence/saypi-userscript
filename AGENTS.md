...
- `doc/layer35-real-host-loop.md` — How to run **Layer 3.5** (`npm run layer35:verify`): an agent-launched Playwright loop against the *real* pi.ai (and other non-Cloudflare hosts), unattended after a one-time `layer35:seed` login; drives the mic via the in-extension synthetic source. Claude/ChatGPT are out of scope here (Cloudflare) — use Layer 4 (CDP).
- `doc/layer4-cdp-real-host-loop.md` — How to run **Layer 4 (CDP)** (`npm run layer4cdp:verify`): an agent-launched real-Chrome-over-CDP loop for the Cloudflare-gated **claude.ai/chatgpt.com**. Verified working **headed** (visible window, agent-driven per run; headless is Cloudflare-blocked) after a one-time profile setup (`layer4cdp:seed` + Developer-mode/Load-unpacked); `layer4cdp:diagnose` confirms usability.
